...
public class Mirror<T> {
    ...
    public Method getSetter(Field field) throws NoSuchMethodException {
        try {
            try {
                return klass.getMethod("set" + StringUtil.firstUpperCase(field.getName()), field.getType());
            } catch (RuntimeException e) {
                try {
            ...
            throw new NoSuchMethodException(String.format("Fail to find setter for [%s]->[%s]",
                    klass.getName(), field.getName()));
        }
    }
     ...
     * @param name 字段名
     * @return 字段
     * @throws NoSuchFieldException 没找到字段
     */
    public Field getField(String name) throws NoSuchFieldException {
        ...
        throw new NoSuchFieldException(String.format(
                "Can not find field [%s] in class [%s] and it's parents classes", name, klass.getName()));
    }
    ...
    /**
     * shashijie 2017-01-19 获取类中对象的所有属性,包括继承自父类的
     * @param noStatic 传入true表示获取静态属性
     * @param noMember 传入true表示获取非静态属性
     * @param noFinal 传入true表示获取final属性
     * @param noInner 传入true表示获取内部类属性
     * @return Field[]
     */
     ...
     * @return
     */
    public <A extends Annotation> A getAnnotation(Method method, Class<A> annType) {
        if (method == null) {
            return null;
        }
    ...
    /**
     * shashijie 2017-01-19 获取类中的方法Method对象 getMethod第一个参数是方法名，第二个参数是该方法的参数类型
    ...
    public void setValue(Object obj, Field field, Object value) {
        ...
        try {
            ...
            try {
                ...
                throw new RuntimeException(String.format("Fail to set value [%s] to [%s]->[%s] because '%s'",
                        value, obj.getClass().getName(), field.getName(), e.getMessage()), e);
            }
        }
    }
    ...
    public void setValue(Object obj, String fieldName, Object value) {
        if (null == value) {
            try {
                ...
                throw new RuntimeException(String.format("Fail to set value [%s] to [%s]->[%s] because '%s'",
                        value, obj.getClass().getName(), fieldName, e.getMessage()), e);
            }
        } else {
            try {
                ...
                try {
                    ...
                    throw new RuntimeException(String.format(
                            "Fail to set value [%s] to [%s]->[%s] because '%s'", value, obj.getClass()
                                    .getName(), fieldName, e.getMessage()), e1);
                }
            }
        }
    }
    ...
    public Object getValue(Object obj, Field file) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("Fail to get value for [%s]->[%s]", obj.getClass(),
                    file.getName()), e);
        }
    }
    ...
    public boolean isDateTimeLike() {
        return Calendar.class.isAssignableFrom(klass) || java.util.Date.class.isAssignableFrom(klass)
                || java.sql.Date.class.isAssignableFrom(klass) || java.sql.Time.class.isAssignableFrom(klass);
    }
    ...
    public static void populateBean(Object obj, Map<String, Object> properties) {
        ...
        for (Field f : mirror.getFields()) {
            // 如果为NULL或者空字符串则为初始值
            if (properties.get(f.getName()) != null && !properties.get(f.getName()).equals("")) {
    ...
    public static Object clone(Object fromObject) {
        ...
        try {
            ...
            for (int j = 0; j < fields.length; j++) {
                ...
                // 得到get,set属性方法的名字
                String getMethodName = "get" + stringLetter + fieldName.substring(1);
                String setMethodName = "set" + stringLetter + fieldName.substring(1);

                Method getMethod = fromClassType.getMethod(getMethodName, new Class[] {});
                Method setMethod = toClassType.getMethod(setMethodName, new Class[] { field.getType() });
     ...
     * @return
     */
    public static <F> Class<?> getParentClass(Class<F> type, Class<?> parentClass) {
        Class<?> cc = type;
    ...
    public static Object parseObj(Class<?> clazz, Object o) throws Exception {
        ...
        // 如果值是数组，目标属性类型也是数组
        if (o.getClass().isArray() && clazz.isArray()) {
            return o;// 直接返回
        }
        // 如果值是对象，目标属性是List,返回List集合对象
        if (o.getClass().isArray() && clazz.isAssignableFrom(List.class)) {
    ...
    /**
     * shashijie 2017-01-19 循环获取对象下的所有属性，包括继承自父类的
     * @param calzz 类实例
     * @param noStatic 传入true表示获取静态属性
     * @param noMember 传入true表示获取非静态属性
     * @param noFinal 传入true表示获取final属性
     ...
     * @return Field[]
     */
    public static Field[] getAllFields(Class<?> clazz, boolean noStatic, boolean noMember, boolean noFinal,
            boolean noInner) {
        // LinkedHashMap存放类中所有属性
        Map<String, Field> map = new LinkedHashMap<String, Field>();
        // 递归循环获取,直到对象为Object或空
        while (null != clazz && clazz != Object.class) {
            ...
            for (int i = 0; i < fs.length; i++) {
                ...
                // 存入map集合:key=属性名,value=属性Field对象
                map.put(f.getName(), f);
            }
            // 获取当前实体类的父类
